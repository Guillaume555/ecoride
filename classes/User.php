...
class User
{
    ...
    public function register($username, $email, $password, $phone = null)
    {
        ...
        // CORRECTION : Vérifier les doublons AVANT insertion
        ...
        try {
            ...
            INSERT INTO users (username, email, password, phone, credits, role, created_at) 
            VALUES (:username, :email, :password, :phone, 20, 'passenger', NOW())
        ");
            ...
            // En cas d'erreur SQL inattendue (ne devrait plus arriver avec les vérifications)
    ...
    public function update($username, $email, $phone = null)
    {
        ...
        try {
            $stmt = $this->pdo->prepare("
                UPDATE users 
                SET username = :username, email = :email, phone = :phone, updated_at = NOW()
                WHERE id = :id
            ");
    ...
    public function unban()
    {
        ...
        try {
            $stmt = $this->pdo->prepare("
                UPDATE users 
                SET is_banned = 0, updated_at = NOW() 
                WHERE id = :id
            ");
            $stmt->execute([':id' => $this->id]);
    ...
    public function updateCredits($newAmount)
    {
        ...
        try {
            $stmt = $this->pdo->prepare("
                UPDATE users 
                SET credits = :amount, updated_at = NOW() 
                WHERE id = :id
            ");
    ...
    public function getTripsAsPassenger()
    {
        ...
        $stmt = $this->pdo->prepare("
            SELECT b.*, t.departure_city, t.arrival_city, t.departure_time,
                   u.username as driver_name, v.brand, v.model
            FROM bookings b
    ...
    public function getAverageRating()
    {
        ...
        try {
            // Vérifier quelle colonne existe dans ta table reviews
            $stmt = $this->pdo->prepare("
        ...
        ");
            ...
            // Si la table n'existe pas encore ou structure différente, retourner 0
            return 0;
        }
    }
